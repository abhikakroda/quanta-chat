 ...
 *
 * The exported tool names match the Swiggy Food MCP server's tool catalogue.
 * Each tool is mapped to a server (food or instamart) below.
 */
...
function serverFor(tool: string): "food" | "im" {
  ...
  // Default to food — most addressing tools live there.
  return "food";
}

// Cache of recent orders for the UI status bar
...
function fnUrl() {
  ...
  return `${base}/functions/v1/swiggy-mcp`;
}
...
/**
 * Extract the canonical payload from an MCP tool result envelope.
 * MCP results look like: { content: [{type:"text", text:"..."}], structuredContent?, isError? }
 */
...
function rememberOrders(name: string, payload: unknown) {
  ...
  if (name === "get_food_orders" && Array.isArray(p.orders)) {
    ...
    const existing = cachedOrders.findIndex((x) => x.order_id === o.order_id);
    if (existing >= 0) cachedOrders[existing] = o;
...
/**
 * Call a Swiggy MCP tool. Throws SwiggyAuthError on 401/expired tokens so the UI
...
export async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!resp.ok) {
    return { error: data?.error ?? `Tool failed (${resp.status})` };
  }
  if (data.needs_reauth) {
    throw new SwiggyAuthError(server, String(data.reason ?? "unknown"));
  }
...
// ────────────────────────────────────────────────────────────────────────────
// (Legacy mock removed — kept type stubs above so the rest of the app compiles.)
// ────────────────────────────────────────────────────────────────────────────
